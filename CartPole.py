...
class PPO:
    ...
    def compute_advantages(self,rewards,dones,values,next_value):
        ...
        values=values.squeeze().detach().cpu().numpy().tolist()
    ...
    def update(self):
        ...
        for epoch in range(epochs):
            for indices in sampler:
                ...
                policy_loss=-torch.min(val1,val2).mean()-0.01*entropy
...
agent=PPO(state_dim,action_dim,lr,gamma,lambdav,clips)
...
for episode in range(episodes):
    ...
    while not done:
        ...
        agent.transaction((state,action,log_prob,reward,done))
        state=next_state
        episode_reward+=reward
        step_count+=1

        if step_count%timestamp==0 and len(agent.memory)>0:
            agent.update()

    running_reward+=episode_reward

    if (episode+1)%loginterval==0:
        print(f"Episode {episode+1} | Average Reward: {running_reward/loginterval:.2f}")
        running_reward=0
